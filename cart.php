...
<section class="shopping-cart">
    ...
    <div  class="box">
        ...
        <div class="sub-total"> subtotaal : <span>$<?php echo $sub_total = ($fetch_cart['price'] * $fetch_cart['quantity']); ?>/-</span> </div>
    </div>
    <?php
    $grand_total += $sub_total;
        }
    }else{
        echo '<p class="empty">uw winkelmandje is leeg</p>';
    }
    ?>
    </div>

    <div class="more-btn">
        <a href="cart.php?delete_all" class="delete-btn <?php echo ($grand_total > 1)?'':'disabled' ?>" onclick="return confirm('delete all from cart?');">alles verwijderen</a>
    </div>

    <div class="cart-total">
        <p>eindtotaal : <span>$<?php echo $grand_total; ?>/-</span></p>
        <a href="shop.php" class="option-btn">doorgaan met winkelen</a>
        <a href="checkout.php" class="btn  <?php echo ($grand_total > 1)?'':'disabled' ?>">Ga naar de kassa</a>
    </div>
